...
AIRCRAFT_PROFILES_DIR="${ROOT_DIR}/aircraft-profiles"
...
usage() {
  ...
  printf '%s\n' 'Builds atlas-agent_<version>_arm64.deb with MAVSDK and Hailo.'
...
resolve_bytetrack_worker() {
  ...
  for candidate in \
    "${CACHE_DIR}/atlas-bytetrack-worker-linux-arm64"; do
  ...
  BYTETRACK_WORKER_BIN="${CACHE_DIR}/atlas-bytetrack-worker-linux-arm64"
...
mkdir -p "${PACKAGE_ROOT}/usr/share/atlas-agent/aircraft-profiles"
shopt -s nullglob
aircraft_profiles=("${AIRCRAFT_PROFILES_DIR}"/*.json)
shopt -u nullglob
if [[ ${#aircraft_profiles[@]} -eq 0 ]]; then
  printf 'no aircraft profiles found in %s\n' "${AIRCRAFT_PROFILES_DIR}" >&2
  exit 1
fi
for aircraft_profile in "${aircraft_profiles[@]}"; do
  install -m 0644 \
    "${aircraft_profile}" \
    "${PACKAGE_ROOT}/usr/share/atlas-agent/aircraft-profiles/$(basename "${aircraft_profile}")"
done
