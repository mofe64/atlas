...
vector<STrack> BYTETracker::update(const vector<Object>& objects, const CAMERA_MOTION* camera_motion)
{
	...
	if (objects.size() > 0)
	{
		for (int i = 0; i < objects.size(); i++)
		{
			...
			STrack strack(STrack::tlbr_to_tlwh(tlbr_), score, objects[i].detection_index);
	...
	{
		STrack::multi_camera_motion(strack_pool, *camera_motion);
		STrack::multi_camera_motion(unconfirmed, *camera_motion);
	}
