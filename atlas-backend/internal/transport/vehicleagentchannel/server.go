...
func (s *Server) receive(stream pb.VehicleAgentChannelService_ConnectServer, conn *connection) error {
	...
	for {
		...
		perceptionEvent := msg.GetPerceptionEvent()
		if perceptionEvent != nil {
			s.recordPerceptionEvent(stream.Context(), agentID, perceptionEvent)
			continue
		}

		perceptionHealth := msg.GetPerceptionHealth()
		if perceptionHealth != nil {
			s.recordPerceptionHealth(stream.Context(), agentID, perceptionHealth)
			continue
		}
	}
}
...
// recordPerceptionEvent stores compact detection metadata reported by the
// onboard perception process. The associated video stays on RTSP/WebRTC.
func (s *Server) recordPerceptionEvent(ctx context.Context, agentID string, event *pb.PerceptionEvent) {
	if s.hub.deps.Perception == nil {
		s.hub.logger.Warn("vehicle-agent gRPC perception event dropped; service unavailable", "vehicle_agent_id", agentID)
		return
	}

	model, err := perceptionEventToModel(event)
	if err != nil {
		s.hub.logger.Warn("vehicle-agent gRPC perception event rejected", "vehicle_agent_id", agentID, "error", err)
		return
	}

	recorded, err := s.hub.deps.Perception.RecordPerceptionEvent(ctx, agentID, model, time.Now().UTC())
	if err != nil {
		s.hub.logger.Warn("vehicle-agent gRPC perception event rejected", "vehicle_agent_id", agentID, "drone_id", model.DroneID, "error", err)
		return
	}

	s.hub.logger.Info("vehicle-agent gRPC perception event accepted", "vehicle_agent_id", agentID, "drone_id", recorded.DroneID, "source_id", recorded.VideoSourceID, "detections", len(recorded.Detections))
}

// recordPerceptionHealth updates the latest runtime health for the perception
// service. Health is live state and is not persisted to perception_events.
func (s *Server) recordPerceptionHealth(ctx context.Context, agentID string, health *pb.PerceptionHealth) {
	if s.hub.deps.Perception == nil {
		s.hub.logger.Warn("vehicle-agent gRPC perception health dropped; service unavailable", "vehicle_agent_id", agentID)
		return
	}

	model, err := perceptionHealthToModel(health)
	if err != nil {
		s.hub.logger.Warn("vehicle-agent gRPC perception health rejected", "vehicle_agent_id", agentID, "error", err)
		return
	}

	if err := s.hub.deps.Perception.RecordPerceptionHealth(ctx, agentID, model, time.Now().UTC()); err != nil {
		s.hub.logger.Warn("vehicle-agent gRPC perception health rejected", "vehicle_agent_id", agentID, "drone_id", model.DroneID, "error", err)
		return
	}
}
...
func perceptionEventToModel(event *pb.PerceptionEvent) (models.PerceptionEvent, error) {
	observedAt, err := time.Parse(time.RFC3339Nano, event.GetObservedAt())
	if err != nil {
		return models.PerceptionEvent{}, errors.New("observed_at must be an RFC3339 timestamp")
	}

	if strings.TrimSpace(event.GetSourceId()) == "" {
		return models.PerceptionEvent{}, errors.New("source_id is required")
	}
	if strings.TrimSpace(event.GetModelName()) == "" {
		return models.PerceptionEvent{}, errors.New("model_name is required")
	}
	if event.GetInferenceLatencyMs() < 0 {
		return models.PerceptionEvent{}, errors.New("inference_latency_ms must be non-negative")
	}

	detections := make([]models.PerceptionDetection, 0, len(event.GetDetections()))
	for _, detection := range event.GetDetections() {
		item, err := perceptionDetectionToModel(detection)
		if err != nil {
	...
	return models.PerceptionEvent{
		DroneID:            event.GetDroneId(),
		VideoSourceID:      event.GetSourceId(),
		ObservedAt:         observedAt.UTC(),
...
func perceptionDetectionToModel(detection *pb.PerceptionDetection) (models.PerceptionDetection, error) {
	if detection == nil {
		return models.PerceptionDetection{}, errors.New("detection cannot be empty")
	}
	if strings.TrimSpace(detection.GetClassName()) == "" {
		return models.PerceptionDetection{}, errors.New("detection class is required")
	}
	if detection.GetConfidence() < 0 || detection.GetConfidence() > 1 {
		return models.PerceptionDetection{}, errors.New("detection confidence must be between 0 and 1")
	}
	bbox := detection.GetBbox()
	if bbox == nil {
		return models.PerceptionDetection{}, errors.New("detection bbox is required")
	}
	if bbox.GetX() < 0 || bbox.GetX() > 1 || bbox.GetY() < 0 || bbox.GetY() > 1 || bbox.GetW() < 0 || bbox.GetW() > 1 || bbox.GetH() < 0 || bbox.GetH() > 1 {
		return models.PerceptionDetection{}, errors.New("detection bbox values must be normalized between 0 and 1")
	}
	if bbox.GetX()+bbox.GetW() > 1.000001 || bbox.GetY()+bbox.GetH() > 1.000001 {
		return models.PerceptionDetection{}, errors.New("detection bbox must fit inside normalized frame bounds")
	}

	return models.PerceptionDetection{
		ClassName:  detection.GetClassName(),
		Confidence: detection.GetConfidence(),
		BBox:       [4]float64{bbox.GetX(), bbox.GetY(), bbox.GetW(), bbox.GetH()},
	}, nil
}

func perceptionHealthToModel(health *pb.PerceptionHealth) (models.PerceptionHealth, error) {
	lastFrameAt, err := parseOptionalTime(health.GetLastFrameAt(), "last_frame_at")
	if err != nil {
		return models.PerceptionHealth{}, err
	}
	lastDetectionAt, err := parseOptionalTime(health.GetLastDetectionAt(), "last_detection_at")
	if err != nil {
		return models.PerceptionHealth{}, err
	}
	if strings.TrimSpace(health.GetSourceId()) == "" {
		return models.PerceptionHealth{}, errors.New("source_id is required")
	}
	if health.GetFps() < 0 {
		return models.PerceptionHealth{}, errors.New("fps must be non-negative")
	}

	return models.PerceptionHealth{
		DroneID:          health.GetDroneId(),
		SourceID:         health.GetSourceId(),
		InputConnected:   health.GetInputConnected(),
		OutputPublishing: health.GetOutputPublishing(),
		ModelLoaded:      health.GetModelLoaded(),
		Accelerator:      health.GetAccelerator(),
		FPS:              health.GetFps(),
		DroppedFrames:    health.GetDroppedFrames(),
		LastFrameAt:      lastFrameAt,
		LastDetectionAt:  lastDetectionAt,
		LastError:        health.GetLastError(),
		ModelName:        health.GetModelName(),
