...
// PerceptionRepository stores selected detection metadata. It intentionally
// stores compact event metadata only; video frames remain on the RTSP/WebRTC path.
type PerceptionRepository interface {
	GeneratePerceptionEventID(ctx context.Context) (string, error)
	InsertPerceptionEvent(ctx context.Context, event models.PerceptionEvent) error
	ListPerceptionEventsForDrone(ctx context.Context, droneID string, limit int) ([]models.PerceptionEvent, error)
	LatestPerceptionEventForDrone(ctx context.Context, droneID string) (models.PerceptionEvent, bool, error)
}
