# Atlas Agent (Deprecated)

This directory preserves the previous PX4/MAVSDK/gRPC prototype while the new
implementation is built in `../atlas-agent`. Existing hardware and SITL runbooks
continue to use this code until their workflows are migrated.
...
```text
atlas-agent-deprecated/internal/vehicle
```
...
Additional installer examples from the `atlas-agent-deprecated/` directory:
...
```text
atlas-agent-deprecated/internal/mavlinkobserver
```
